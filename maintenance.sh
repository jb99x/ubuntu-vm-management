...
' "${DO_SCHEDULED_REBOOT}"
    ...
    # Safety / sudo
    # -------------------------------
    # If invoked as root (e.g., via systemd), make "sudo <cmd>" a no-op wrapper.
# This keeps the script compatible with both interactive (non-root) and unattended (root) runs.
if [[ "$EUID" -eq 0 ]]; then
  sudo() { "$@"; }
fi

# Interactive usage: require a normal user + sudo to reduce foot-guns.
# Unattended/systemd usage: running as root is expected and supported.
if [[ "$EUID" -eq 0 && "${UNATTENDED}" != "true" && "${INSTALL_TIMER}" != "true" && "${DO_SELF_UPDATE}" != "true" ]]; then
  die "Run as a normal user with sudo, not root."
fi

# Warm up sudo for interactive runs so we don't get interrupted mid-script.
if [[ "$EUID" -ne 0 ]]; then
  sudo -v
fi
