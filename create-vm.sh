...
#
# v1.1.0: Default VM disk mode switched to 'copy' for long-lived VMs.
#         Persisted disk mode in profile.
# ==============================================================================
...
# Copy mode is recommended for long-lived VMs (no dependency on a base image path/permissions at boot).
# Overlay mode is best for ephemeral/test VMs but requires the backing image to remain accessible.
DISK_MODE_DEFAULT="copy"                 # overlay|copy
...
save_create_profile() {
  ...
  echo "  UBUNTU_CODENAME_DEFAULT=${UBUNTU_CODENAME_DEFAULT}"
  echo "  IMAGE_FLAVOR_DEFAULT=${IMAGE_FLAVOR_DEFAULT}"
  echo "  OS_VARIANT_DEFAULT=${OS_VARIANT_DEFAULT}"
  echo "  TIMEZONE_DEFAULT=${TIMEZONE_DEFAULT}"
  echo "  LAN_SSH_CIDR_DEFAULT=${LAN_SSH_CIDR_DEFAULT}"
  echo "  VPN_SSH_CIDR_DEFAULT=${VPN_SSH_CIDR_DEFAULT}"
  echo "  RAM_MB_DEFAULT=${RAM_MB_DEFAULT}"
  echo "  MAX_RAM_MB_DEFAULT=${MAX_RAM_MB_DEFAULT}"
  echo "  VCPUS_DEFAULT=${VCPUS_DEFAULT}"
  echo "  MAX_VCPUS_DEFAULT=${MAX_VCPUS_DEFAULT}"
  echo "  DISK_SIZE_GB_DEFAULT=${DISK_SIZE_GB_DEFAULT}"
  echo "  DISK_MODE_DEFAULT=${DISK_MODE_DEFAULT}"
  echo "  FORCE_DHCP_NETCFG_DEFAULT=${FORCE_DHCP_NETCFG_DEFAULT}"
  echo "  WAIT_FOR_IP_DEFAULT=${WAIT_FOR_IP_DEFAULT}"
  echo "  WAIT_FOR_IP_TIMEOUT_DEFAULT=${WAIT_FOR_IP_TIMEOUT_DEFAULT}"
  echo "  VNC_DEFAULT=${VNC_DEFAULT}"
  echo "  VNC_LISTEN_DEFAULT=${VNC_LISTEN_DEFAULT}"
  echo "  RUN_LOG_DIR_DEFAULT=${RUN_LOG_DIR_DEFAULT}"
  echo "  CLEANUP_VM_CLOUDINIT_DIR_DEFAULT=${CLEANUP_VM_CLOUDINIT_DIR_DEFAULT}"
  echo "  KEEP_UBUNTU_BOOT_DIRS_DEFAULT=${KEEP_UBUNTU_BOOT_DIRS_DEFAULT}"
  echo

  confirm_yn "Save these defaults to ${CREATE_PROFILE_PATH}?" || { echo "Skipped saving create-profile."; return 0; }
  ...
  tmp="$(mktemp)"
  {
    ...
    echo
    printf 'UBUNTU_CODENAME_DEFAULT=%q\n' "${UBUNTU_CODENAME_DEFAULT}"
    printf 'IMAGE_FLAVOR_DEFAULT=%q\n' "${IMAGE_FLAVOR_DEFAULT}"
    printf 'OS_VARIANT_DEFAULT=%q\n' "${OS_VARIANT_DEFAULT}"
    printf 'TIMEZONE_DEFAULT=%q\n' "${TIMEZONE_DEFAULT}"
    printf 'LAN_SSH_CIDR_DEFAULT=%q\n' "${LAN_SSH_CIDR_DEFAULT}"
    printf 'VPN_SSH_CIDR_DEFAULT=%q\n' "${VPN_SSH_CIDR_DEFAULT}"
    printf 'RAM_MB_DEFAULT=%q\n' "${RAM_MB_DEFAULT}"
    printf 'MAX_RAM_MB_DEFAULT=%q\n' "${MAX_RAM_MB_DEFAULT}"
    printf 'VCPUS_DEFAULT=%q\n' "${VCPUS_DEFAULT}"
    printf 'MAX_VCPUS_DEFAULT=%q\n' "${MAX_VCPUS_DEFAULT}"
    printf 'DISK_SIZE_GB_DEFAULT=%q\n' "${DISK_SIZE_GB_DEFAULT}"
    printf 'DISK_MODE_DEFAULT=%q\n' "${DISK_MODE_DEFAULT}"
    printf 'FORCE_DHCP_NETCFG_DEFAULT=%q\n' "${FORCE_DHCP_NETCFG_DEFAULT}"
    printf 'WAIT_FOR_IP_DEFAULT=%q\n' "${WAIT_FOR_IP_DEFAULT}"
    printf 'WAIT_FOR_IP_TIMEOUT_DEFAULT=%q\n' "${WAIT_FOR_IP_TIMEOUT_DEFAULT}"
    printf 'VNC_DEFAULT=%q\n' "${VNC_DEFAULT}"
    printf 'VNC_LISTEN_DEFAULT=%q\n' "${VNC_LISTEN_DEFAULT}"
    printf 'RUN_LOG_DIR_DEFAULT=%q\n' "${RUN_LOG_DIR_DEFAULT}"
    printf 'CLEANUP_VM_CLOUDINIT_DIR_DEFAULT=%q\n' "${CLEANUP_VM_CLOUDINIT_DIR_DEFAULT}"
    printf 'KEEP_UBUNTU_BOOT_DIRS_DEFAULT=%q\n' "${KEEP_UBUNTU_BOOT_DIRS_DEFAULT}"
    echo
  } > "${tmp}"
...
prompt_with_default DISK_MODE "Disk mode (copy recommended for long-lived; overlay for ephemeral) (overlay|copy)" "${DISK_MODE_DEFAULT}"
